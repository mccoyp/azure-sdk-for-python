...
def _get_certificates_path_prefix() -> str:
    hsm_url = os.environ["AZURE_MANAGEDHSM_URL"]
    hsm_name = urlparse(hsm_url).netloc.split(".")[0]
    return f"{PATH_PREFIX}/{hsm_name}-certificate"


def _get_security_domain_location() -> str:
    secondary_hsm_url = os.environ["SECONDARY_MANAGEDHSM_URL"]
    secondary_hsm_name = urlparse(secondary_hsm_url).netloc.split(".")[0]
    return f"{PATH_PREFIX}/{secondary_hsm_name}-security-domain.json"


def _get_transfer_key_location() -> str:
    secondary_hsm_url = os.environ["SECONDARY_MANAGEDHSM_URL"]
    secondary_hsm_name = urlparse(secondary_hsm_url).netloc.split(".")[0]
    return f"{PATH_PREFIX}/{secondary_hsm_name}-transfer-key.pem"


def _get_wrapping_key_locations() -> list[str]:
    certs_path = _get_certificates_path_prefix()
    return [f"{certs_path}0.pem", f"{certs_path}1.pem"]
...
def use_wrapping_keys() -> dict:
    security_domain = _get_security_domain_location()
    transfer_key = _get_transfer_key_location()
    key_paths = _get_wrapping_key_locations()
    blob_value = _security_domain_restore_blob(security_domain, transfer_key, key_paths)
    return {'value': blob_value}


def write_security_domain(security_domain: str) -> None:
    sd_path = _get_security_domain_location()
    try:
...
def write_transfer_key(transfer_key: dict) -> None:
    key_path = _get_transfer_key_location()
