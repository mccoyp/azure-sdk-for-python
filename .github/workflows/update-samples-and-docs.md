...
This repository uses a snippet-generation tool that extracts code from sample files into README documentation. The coding agent does NOT manually paste code into README — the tooling does that. The flow is described in `doc/dev/code_snippets.md`.

Follow the `code_snippets.md` guidance to add and/or update snippets and run `azpysdk update_snippet .` from the package directory to update snippets in documentation.
...
Use `sdk/keyvault/azure-keyvault-secrets/README.md` as the canonical reference for section formatting and tone
...
1. `cd sdk/<service>/<package>`
2. `azpysdk update_snippet .` — this reads snippet tags from sample files and injects the code into README.md placeholder blocks. Check the git diff after running to confirm README was updated correctly
3. `eng/scripts/Export-API.ps1 <service-directory>` — regenerates public API listing files under `sdk/<service>/<package>/api/`. Required if any public types or members were added or changed
4. `azpysdk black .` — ensures consistent code formatting
...
- File at most one issue per push; scope to the most impactful documentation gap, if one exists at all
